 ...
 */
export function extractPageKeys(placeholder: string): Uint8Array[] {
  ...
  while (pos + 2 <= raw.length) {
    ...
    if (length <= 0 || end > raw.length) break;
...
export function encodeKeyFragment(key: Uint8Array): string {
  let hex = "";
...
export function parseKeyFragment(url: string): Uint8Array | undefined {
 ...
 */
export async function descrambleAlphaManga(
  data: ArrayBuffer,
  key: Uint8Array,
  mimeType: string,
): Promise<ArrayBuffer> {
  if (key.length < 8 || key.length % 8 !== 0) return data;

  const v0 = readU32LE(key, 0);
  const ha0 = readU32LE(key, 4);
  const tileSize = (ha0 >>> 24) & 0xff;
  const w = (v0 >>> 27) & 7;
  if (tileSize === 0) return data;
  ...
  if (!srcWidth || !srcHeight) return data;
  ...
  for (let idx = 0; idx < tileCount; idx++) {
    ...
    const spanX =
      (t !== 0 && Math.floor(destLeft / t) === lastCol ? outW - destLeft : t) + r;
    const spanY =
      (t !== 0 && Math.floor(destTop / t) === lastRow ? outH - destTop : t) + r;
    ...
    if (rot !== 0) ctx.rotate((-90 * rot * Math.PI) / 180);
