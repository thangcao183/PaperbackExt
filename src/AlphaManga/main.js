...
import { descrambleAlphaManga, encodeKeyFragment, extractPageKeys, parseKeyFragment, } from "./descramble";
import { AlphaMangaSearchForm } from "./forms";
...
const MOBILE_UA = "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 " +
...
export class AlphaMangaExtension {
    ...
    async getDiscoverSectionItems(_section, metadata) {
        ...
        const items = result.entries.map((m) => ({
    ...
    async getSearchResults(query, metadata) {
        ...
        const result = await this.fetchSearch(page, (query.title || "").trim(), searchMeta?.status?.[0] ?? "", searchMeta?.genre?.[0] ?? "");
        const items = result.entries.map((m) => ({
            mangaId: this.toSafeId(String(m.manga_sele_id)),
            imageUrl: m.banner_image_url ?? "",
            title: m.title,
    ...
    async getMangaDetails(mangaId) {
        ...
        const tagGroups = genres.length > 0
            ...
            : [];
        const thumbnailUrl = $("img.p-manga-detail__banner-image").first().attr("src") || "";
        return {
            mangaId,
            mangaInfo: {
                primaryTitle: title,
                ...
                status: this.parseStatus($(".p-manga-detail__status p").first().text().trim()),
                tagGroups,
    ...
    async getChapterDetails(chapter) {
        ...
        if (!raw) {
            throw new Error("Log in via the browser and rent or purchase this chapter to read it.");
        }
        ...
        const urls = parsed.filter((p) => typeof p === "string" && p !== "first" && p !== "last");
    ...
    // Helpers
    // ----------------------------------------------------------------
    async fetchSearch(page, query, status, genre) {
        const url = `${BASE_URL}/manga/search.json` +
