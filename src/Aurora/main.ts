...
type AuroraImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class AuroraExtension implements AuroraImplementation {
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    const pages: string[] = [];
    const src =
      $(".webcomic-media .webcomic-link .attachment-full").first().attr("src") ||
      "";
  ...
  private async fetchChaptersAsMangas(): Promise<
  ...
  private async titleForManga(mangaId: string): Promise<string> {
    const entries = await this.fetchChaptersAsMangas();
  ...
  private async statusForChapter(chapter: number): Promise<string> {
