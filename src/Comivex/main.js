...
import { ComivexSearchForm } from "./forms";
...
export class ComivexExtension {
        ...
        if (section.id === "latest") {
            ...
            $("article.u-card").each((_, element) => {
                ...
                items.push({
                    ...
                    mangaId,
                    imageUrl: this.absoluteUrl(card.find("img.u-card__img").first().attr("src") || ""),
                    title,
                    metadata: undefined,
                });
            });
    ...
    async exploreParse(url, sort) {
        ...
        $("article.manga-card").each((_, element) => {
            ...
            if (applyStaleFilter) {
                ...
                if (idMatch && STALE_EXPLORE_IDS.has(idMatch[1]))
                    return;
            }
    ...
    async getMangaDetails(mangaId) {
        ...
        const thumbnailUrl = this.absoluteUrl($(".md-cover-wrap img.md-cover").first().attr("src") || "");
