...
function thumbnailForTitle(nthManga) {
    switch (nthManga) {
        case 0:
...
const PAGE_DATE_FMT = /^\w+,\s+(\d+)\s+(\w+),\s+(\d+);\s+(\d+):(\d+):(\d+)\s+\w+$/;
const MONTHS = {
    jan: 0,
    feb: 1,
    mar: 2,
...
export class DarthsDroidsExtension {
    ...
    async getMangaDetails(mangaId) {
        ...
        if (!book) {
            book = books.find((b) => this.safeDecode(b.mangaId) === slug);
        }
