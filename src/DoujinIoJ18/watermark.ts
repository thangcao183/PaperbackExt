...
/** Minimal structural type for the WebCrypto subtle API used here. */
 ...
 */
function extractCiphertext(data: Uint8Array): Uint8Array | undefined {
  ...
  while (offset + 1 < data.length) {
    ...
    if (length < 2 || payloadEnd > data.length) break;
...
async function aesEcbDecrypt(
  ...
  if (ciphertext.length === 0 || ciphertext.length % 16 !== 0) {
    throw new Error(
      "Watermark ciphertext is not a multiple of the AES block size",
    );
  }
  ...
  for (let i = 0; i < 16; i++) padSource[i] = 0x10 ^ lastBlock[i];
...
export async function removeWatermark(
  ...
  if (!ciphertext) return data;

  const plaintext = await aesEcbDecrypt(ciphertext, keyBytes);
  if (plaintext.length < 20) return data;
  ...
  if (!baseWidth || !baseHeight) return data;

  const overlay = await loadImageFromBuffer(
    bufferOf(overlayBytes),
    "image/jpeg",
  );
