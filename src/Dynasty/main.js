...
function buildCoverUrlStr(file) {
...
function buildCoverFetchUrl(directory, permalink) {
    return (`${BASE_URL}${COVER_FETCH_PATH}` +
...
export class DynastyExtension {
    ...
    async getDiscoverSectionItems(section, _metadata) {
        ...
        $("a.chapter.media.thumbnail").each((_, element) => {
            ...
            if (!href || !href.includes(`/${CHAPTERS_DIR}/`))
                return;
    ...
    async getSearchResults(query, metadata) {
        ...
        const hrefRegex = new RegExp(`/(${SERIES_DIR}|${ANTHOLOGIES_DIR}|${CHAPTERS_DIR}|${DOUJINS_DIR}|${ISSUES_DIR})/`);
        const selector = ".chapter-list a.name, .chapter-list .doujin_tags a[href]";
    ...
    async resolveSearchCover(directory, permalink) {
        try {
    ...
    async getMangaDetails(mangaId) {
        ...
        const publishingStatus = new Set();
        const otherSections = [];
        const otherMap = new Map();
        const pushOther = (type, name) => {
        ...
        for (const tagging of data.taggings ?? []) {
            if (tagging.header !== undefined && tagging.title === undefined)
                continue;
        ...
        const authorStr = authorList.length > AUTHORS_UPPER_LIMIT
    ...
    buildSynopsis(data, otherSections) {
        const parts = [];
        if (data.description) {
    ...
    async chapterAsManga(mangaId, permalink) {
        ...
        const synopsisParts = [`Type: ${CHAPTER_TYPE}`];
        for (const [type, values] of otherMap) {
            const lines = [`${type}:`];
        ...
        if (data.released_on)
            synopsisParts.push(`Released: ${data.released_on}`);
    ...
    async getChapters(sourceManga) {
        ...
        const data = await this.fetchJson(baseJsonUrl);
        const taggings = [...(data.taggings ?? [])];
        const totalPages = data.total_pages ?? 1;
        let page = 2;
        while (page <= totalPages && page <= CHAPTER_FETCH_LIMIT) {
            const pageData = await this.fetchJson(`${baseJsonUrl}?page=${page}`);
    ...
    async getChapterDetails(chapter) {
        ...
        for (const page of data.pages ?? []) {
            if (page.url)
                pages.push(this.absoluteUrl(page.url));
        }
        return {
    ...
    parseDirAndPermalink(href) {
        ...
        if (!VALID_DIRS.includes(directory))
            return undefined;
        return { directory, permalink };
    }
    mangaUrl(mangaId) {
    ...
    parseStatus(publishingStatus) {
        if (publishingStatus.has("Ongoing"))
            return "Ongoing";
        if (publishingStatus.has("Completed"))
            return "Completed";
        if (publishingStatus.has("On Hiatus"))
            return "Hiatus";
        if (publishingStatus.has("Licensed"))
            return "Completed";
        for (const s of ["Dropped", "Cancelled", "Not Updated", "Abandoned", "Removed"]) {
            if (publishingStatus.has(s))
                return "Cancelled";
        }
        return "Unknown";
    }
    ...
    decodeUnicode(input) {
        return input.replace(UNICODE_REGEX, (_, hex) => String.fromCharCode(parseInt(hex, 16)));
    }
    // ----------------------------------------------------------------
    // Cloudflare + fetch
    // ----------------------------------------------------------------
