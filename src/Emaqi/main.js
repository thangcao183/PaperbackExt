...
export class EmaqiExtension {
    ...
    async getDiscoverSectionItems(section, metadata) {
        ...
        const data = await this.graphQL(SERIES_QUERY, "FetchHomeSection", {
            slug: section.id,
            mangaAfter: cursor,
        });
        ...
        for (const edge of conn?.edges ?? []) {
            ...
            items.push({
                type: section.id === "this-week-s-bestsellers"
    ...
    async getSearchResults(query, metadata) {
        ...
        if (titleQuery !== "") {
            const data = await this.graphQL(SEARCH_QUERY, "Search", {
                input: { keyword: titleQuery },
            });
            const items = (data.search ?? []).map((comic) => ({
                mangaId: this.buildMangaId(comic),
                imageUrl: comic.cover?.url ?? "",
                title: comic.title,
                subtitle: undefined,
        ...
        const queryGenre = query.metadata?.genre;
        const pageMeta = metadata;
        const genre = pageMeta?.genre ?? queryGenre ?? GENRES[0].slug;
        const cursor = pageMeta?.cursor ?? null;
        const data = await this.graphQL(GENRE_QUERY, "FetchGenre", {
            slug: genre,
            mangaAfter: cursor,
        });
        ...
        const nextMeta = conn?.pageInfo.hasNextPage && conn.pageInfo.endCursor
    ...
    async getMangaDetails(mangaId) {
        ...
        const data = await this.graphQL(DETAILS_QUERY, "FetchMangaStatus", { comicId });
        const comic = data.manga.comic;
        const descriptionParts = [];
        if (comic.synopsis)
            descriptionParts.push(comic.synopsis);
    ...
    async getChapters(sourceManga) {
        ...
        const data = await this.graphQL(CHAPTER_LIST_QUERY, "FetchComicData", { comicId });
        const chapters = [];
        const singles = (data.chapters ?? []).map((c) => this.chapterFromEntry(sourceManga, c, slug));
        // Upstream reverses each list so newest is first.
        singles.reverse();
        chapters.push(...singles);
        const volumes = (data.comicVolumes?.volumes ?? []).map((v) => this.volumeFromEntry(sourceManga, v, slug));
        volumes.reverse();
        chapters.push(...volumes);
        return chapters;
    }
    chapterFromEntry(sourceManga, c, slug) {
    ...
    volumeFromEntry(sourceManga, v, urlSlug) {
    ...
    async getChapterDetails(chapter) {
        ...
        let data;
        if (type === "chapter") {
            data = await this.graphQL(CHAPTER_QUERY, "FetchChapterContents", { comicId, chapterNumber: number }, { "X-Hash": publicKeyB64 });
        }
        else {
            data = await this.graphQL(VOLUME_QUERY, "FetchMangaContents", { comicId, volumeNumber: number }, { "X-Hash": publicKeyB64 });
        }
        const contents = data.chapter?.contents ?? data.manga?.contents ?? null;
        if (!contents || contents.pages.length === 0) {
            throw new Error("No page contents returned. This title likely requires a purchase/login. Locked content cannot be unlocked without an account that owns it.");
        }
        ...
        const pages = contents.pages.map((p) => `${p.url}#${privateKeyB64Url}:${hash}`);
        return {
    ...
    async graphQL(query, operationName, variables, extraHeaders) {
