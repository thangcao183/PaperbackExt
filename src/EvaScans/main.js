...
class EvaScansExtension extends MangaThemesiaExtension {
    async getMangaDetails(mangaId) {
        ...
        if (!synopsis)
            synopsis = $(".synopsis-short p").first().text().trim();
        if (!synopsis) {
        ...
        if (!isNaN(rating) && rating > 0)
            parts.push(`Rating: ${rating.toFixed(2)}/10`);
        const views = stats.get("Views");
        if (views)
            parts.push(`Views: ${views}`);
        if (synopsis)
            parts.push(`Synopsis: ${synopsis}`);
        ...
        return {
            ...
            mangaInfo: {
                ...
                secondaryTitles: altNames.length > 0 ? altNames : base.mangaInfo.secondaryTitles,
