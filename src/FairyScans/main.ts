...
const GENRE_VALUES: { id: string; title: string }[] = [
...
type FairyScansImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class FairyScansExtension implements FairyScansImplementation {
  ...
  async getDiscoverSectionItems(
    ...
    if (section.id === "genres") {
      const items: DiscoverSectionItem[] = GENRE_VALUES.map((g) => ({
    ...
    const meta = metadata as FairyScansMetadata | undefined;
  ...
  async getSearchResults(
    ...
    const queryMeta = query.metadata as FairyScansMetadata | undefined;
  ...
  private async browse(opts: {
    ...
    const action =
      opts.page === 1 ? "greed_filter_series" : "greed_archive_load_more";
    ...
    $("article").each((_, element) => {
      ...
      const formatBadge = el
        ...
        .toLowerCase();
      const isNovel =
        el.find(".greed-browse-card-format-badge--novel").length > 0 ||
        formatBadge === "novel";
      if (isNovel) return;

      const link = el
        .find(
          "h2 a, a.greed-browse-card-image, a.greed-archive-cover",
        )
        .first();
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const title =
      $(".greed-series-title, h1").first().text().trim() ||
    ...
    const statusText = $(
      ".fairy-series-clean__meta-item--status .fairy-series-clean__meta-v",
    )
      .first()
      .text()
      .toLowerCase();
  ...
  async getChapters(sourceManga: SourceManga): Promise<Chapter[]> {
    ...
    const entries: { chapter: Chapter; order: number }[] = [];
    const seen = new Set<string>();
    ...
    $(".greed-series-chapter").each((_, element) => {
      ...
      const orderFallback = parseFloat(
        CHAPTER_NUM_REGEX.exec(rawName)?.[1] ?? "",
      );
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    let dto: ReaderResponse;
    try {
      dto = JSON.parse(jsonStr) as ReaderResponse;
    } catch {
  ...
  private parseRelativeDate(dateStr: string): Date {
    ...
    if (isNaN(count)) return new Date(0);
    const unit = split[1].toLowerCase();
    const now = Date.now();

    let ms: number;
    if (unit.includes("year")) ms = count * 365 * 24 * 60 * 60 * 1000;
    else if (unit.includes("month")) ms = count * 30 * 24 * 60 * 60 * 1000;
    else if (unit.includes("week")) ms = count * 7 * 24 * 60 * 60 * 1000;
    else if (unit.includes("day")) ms = count * 24 * 60 * 60 * 1000;
    else if (unit.includes("hour")) ms = count * 60 * 60 * 1000;
    else if (unit.includes("min")) ms = count * 60 * 1000;
    else if (unit.includes("sec")) ms = count * 1000;
