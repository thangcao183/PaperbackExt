...
class FirescansExtension extends MadaraExtension {
  ...
  private readonly chapterProtectorSelector = "#chapter-protector-data";
  private readonly chapterProtectorPasswordPrefix = "wpmangaprotectornonce='";
  private readonly chapterProtectorDataPrefix = "chapter_data='";
  ...
  override async getChapterDetails(
    ...
    if (src.startsWith(dataPrefix)) {
      protectorHtml = utf8FromBytes(
        base64ToBytes(src.slice(dataPrefix.length)),
      );
    } else {
    ...
    const password = substringBefore(
      substringAfter(protectorHtml, this.chapterProtectorPasswordPrefix),
      "';",
    );

    const chapterDataRaw = substringBefore(
      substringAfter(protectorHtml, this.chapterProtectorDataPrefix),
      "';",
    ).replace(/\\\//g, "/");
