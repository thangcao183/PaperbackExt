...
type HentaiKunImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class HentaiKunExtension implements HentaiKunImplementation {
  ...
  async getDiscoverSectionItems(
    ...
    const meta = metadata as HentaiKunMetadata | undefined;
  ...
  async getSearchResults(
    ...
    const parsed = this.parseListing($);
    const items: SearchResultItem[] = parsed.map((entry) => ({
      mangaId: entry.mangaId,
      imageUrl: entry.imageUrl,
      title: entry.title,
      subtitle: undefined,
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const title =
      $("div.single_title h1").first().text().trim() ||
    ...
    if ($("table.table-striped").length > 0) {
      ...
      $("div.thumbnail[id^='galary-']").each((_, element) => {
        ...
        const imageUrl = this.imageFromElement(
          div.find("img.img-responsive").first(),
        );
        seen.add(mangaId);
        results.push({ mangaId, imageUrl, title });
      });
  ...
  private imageFromTitleAttr(titleHtml: string): string {
    if (!titleHtml) return "";
