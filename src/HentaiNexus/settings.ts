...
function setImageFormat(value: string): void {
  Application.setState(value, IMAGE_FORMAT_KEY);
}

export function getImageFormatLabel(format: string): string {
  return (
    IMAGE_FORMAT_OPTIONS.find((option) => option.id === format)?.title ?? format
  );
}

export function getImageField(format: string): string {
  switch (format) {
    case "source":
      return "image_source";
    case "avif":
...
export class HentaiNexusSettingsForm extends Form {
  ...
  async updateImageFormat(value: string[]): Promise<void> {
