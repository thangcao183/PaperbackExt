...
type IRovedOutImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class IRovedOutExtension implements IRovedOutImplementation {
  ...
  async getChapters(sourceManga: SourceManga): Promise<Chapter[]> {
    ...
    main$(`#menu-menu > li > a[href^="${ARCHIVE_URL}"]`).each((_, element) => {
      ...
      if (href) bookUrls.push(this.absoluteUrl(href));
    });
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    if (match) {
      ...
      const archiveUrl =
        bookNumber !== 1 ? `${ARCHIVE_URL}-book-${bookNumber}` : ARCHIVE_URL;
      ...
      const target = wraps.find(
        (el) =>
          book$(el).find(".comic-archive-chapter").first().text().trim() ===
          chapterTitle,
      );
