...
export class KMangaSettingsForm extends Form {
  constructor(
    private readonly cookieStorageInterceptor: CookieStorageInterceptor,
  ) {
    super();
  }
   ...
   */
  async onLoginComplete(cookies: Cookie[]): Promise<void> {
  ...
  async onLoginCancel(): Promise<void> {
    this.reloadForm();
  }
      ...
      Section(
        {
          id: "account",
          footer:
            "Log in to your Kodansha (K Manga) account to read chapters you " +
