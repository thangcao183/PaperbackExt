...
class KunMangaOnlineExtension extends MadaraExtension {
    ...
    async getSearchResults(query, metadata, sortingOption) {
        ...
        if (searchMeta) {
            if (searchMeta.author)
                params.push(["author", searchMeta.author]);
            if (searchMeta.artist)
                params.push(["artist", searchMeta.artist]);
            if (searchMeta.release)
                params.push(["release", searchMeta.release]);
    ...
    hasNextPaginationLink($) {
        return ($("a[aria-label=Next], .nav-previous, .next.page-numbers, .pagination-next, .wp-pagenavi .next, a[rel=next], .next").length > 0);
    }
