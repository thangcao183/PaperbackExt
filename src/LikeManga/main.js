...
import { LikeMangaSearchForm } from "./forms";
...
export class LikeMangaExtension {
    ...
    async getChapters(sourceManga) {
        ...
        if (!isNaN(id) && lastPage > 1) {
            for (let page = 2; page <= lastPage; page++) {
                try {
                    const ajaxChapters = await this.fetchAjaxChapters(id, page, sourceManga, seen);
    ...
    collectChapters($, root, sourceManga, chapters, seen) {
        $(".wp-manga-chapter", root).each((_, element) => {
            const el = $(element);
    ...
    async fetchAjaxChapters(id, page, sourceManga, seen) {
        const params = [
            "act=ajax",
        ...
        const htmlString = parsed["list_chap"] ?? "";
        if (!htmlString)
            return [];
    ...
    async getChapterDetails(chapter) {
        ...
        if (tokenInput.length > 0) {
            ...
            if (cdnUrl && tokenParts.length > 1) {
                try {
                    ...
                    const encodedImgArray = jsonData["data"];
                    const imgArrayStr = this.base64ToString(encodedImgArray);
                    const imgArray = JSON.parse(imgArrayStr);
        ...
        if (pages.length === 0) {
            $("div.reading-detail.box_doc img:not(noscript img)").each((_, element) => {
