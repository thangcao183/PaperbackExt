...
export class LustToonExtension {
    ...
    async getDiscoverSectionItems(section, metadata) {
        ...
        const resp = await this.fetchJson({ url, method: "GET" });
    ...
    async getSearchResults(query, metadata) {
        ...
        if (titleQuery !== "") {
            ...
            const items = await this.fetchJson({ url, method: "GET" });
            const results = [];
    ...
    async getChapters(sourceManga) {
        ...
        for (const chapter of serie.chapters ?? []) {
            ...
            const title = chapName !== "" && /\d/.test(chapName)
                ? chapName
    ...
    parseStatus(status) {
        ...
        if (s.includes("en emision") || s.includes("ongoing"))
            return "Ongoing";
