...
type MadokamiImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class MadokamiExtension implements MadokamiImplementation {
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const thumbnailUrl = this.imageFromElement(
      $('div.manga-info img[itemprop="image"]').first(),
    );
  ...
  async getChapters(sourceManga: SourceManga): Promise<Chapter[]> {
    ...
    const seen = new Set<string>();
    $("table#index-table > tbody > tr > td:nth-child(6) > a").each(
      (_, element) => {
        const row = $(element).parent().parent();
        const readerHref = row.find("td:nth-child(6) a").attr("href") || "";
        if (!readerHref) return;

        const idx = readerHref.indexOf("/reader");
        const readerPath =
          idx >= 0 ? `/reader${readerHref.substring(idx + "/reader".length)}` : "";
  ...
  private detailsUrl(mangaId: string): string {
  ...
  private titleFromPath(mangaId: string): string {
    const slug = this.safeDecode(mangaId).replace(/^\/+/, "");
    const segments = slug
      .split("/")
      .filter((s) => s.length > 0)
  ...
  private parseDate(date: string): Date {
    ...
    if (d.endsWith("ago")) {
      ...
      const now = Date.now();
      if (unit.startsWith("sec")) return new Date(now - amount * 1000);
      if (unit.startsWith("min")) return new Date(now - amount * 60_000);
      if (unit.startsWith("hour")) return new Date(now - amount * 3_600_000);
      return new Date(0);
    }
