...
import { GENRE_ORDER, MangaFreakSearchForm, } from "./forms";
...
export class MangaFreakExtension {
    ...
    async getSearchResults(query, _metadata) {
        ...
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const results = [];
        $("div.manga_search_item , div.mangaka_search_item").each((_, element) => {
            const el = $(element);
    ...
    buildSearchUrl(query, searchMeta) {
        const segments = [];
        if (query) {
    ...
    async getMangaDetails(mangaId) {
        ...
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const thumbnailUrl = this.absoluteUrl($("div.manga_series_image img").first().attr("src") || "");
