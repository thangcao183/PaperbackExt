...
class MangaGGExtension extends MadaraExtension {
  ...
  override async getDiscoverSectionItems(
    ...
    const meta = metadata as MangaGGMetadata | undefined;
  ...
  private mangaggLoadMoreBody(page: number, popular: boolean): string {
  ...
  override async getChapters(sourceManga: SourceManga): Promise<Chapter[]> {
    ...
    for (;;) {
      ...
      if (xhr(`.pagination a[data-page=${page + 1}]`).length === 0) break;
      page++;
    }
