...
export class MangaGoExtension {
    ...
    async getChapterDetails(chapter) {
        ...
        const pages = await this.resolveAllPages($, finalUrl, availableImages, jsCache);
        return {
            id: chapter.chapterId,
            mangaId: chapter.sourceManga.mangaId,
            pages,
    ...
    async resolveAllPages($, finalUrl, availableImages, jsCache) {
        let totalPages = 0;
        $("script").each((_, el) => {
            if (totalPages)
                return;
    ...
    computePagePrefix(finalUrl, urlTemplate) {
    ...
    // ----------------------------------------------------------------
    async getChapterImageUrls($, jsCache) {
        ...
        if (!keyHex || !ivHex)
            return [];
        const decrypted = await aesCbcDecryptNoPad(encryptedBytes, this.hexToBytes(keyHex), this.hexToBytes(ivHex));
    ...
    async resolveDescramblingKeys(urls, imgKeys) {
        const inject = `
...
`;
    ...
    findHexEncodedVariable(js, name) {
        const regex = new RegExp(`var ${name}\\s*=\\s*CryptoJS\\.enc\\.Hex\\.parse\\("([0-9a-zA-Z]+)"\\)`);
        const match = js.match(regex);
        return match ? match[1] : "";
    }
    unscrambleImageList(imageList, chapterJs) {
        const matches = [...chapterJs.matchAll(KEY_LOCATION_REGEX)].map((m) => parseInt(m[1], 10));
        ...
        if (unscrambleKey.some((n) => isNaN(n)))
            return imageList;
    ...
    extractImgKeys(chapterJs) {
        const afterMarker = chapterJs.split("var renImg = function(img,width,height,id){")[1];
        if (!afterMarker)
            return "";
    ...
    parseChapterNumber(name, chapterId) {
    ...
    async fetchCheerioWithUrl(request) {
...
async function aesCbcDecryptNoPad(ciphertext, key, iv) {
    ...
    for (let i = 0; i < 16; i++)
        padBlock[i] = 0x10 ^ lastBlock[i];
    const zeroIv = new Uint8Array(16);
    const encrypted = new Uint8Array(await subtle.encrypt({ name: "AES-CBC", iv: bufferOf(zeroIv) }, cryptoKey, bufferOf(padBlock)));
    ...
    const decrypted = new Uint8Array(await subtle.decrypt({ name: "AES-CBC", iv: bufferOf(iv) }, cryptoKey, bufferOf(extended)));
