...
import { MangaHereSearchForm, MangaHereSearchMeta } from "./forms";

const BASE_URL = "https://www.mangahere.cc";

interface MangaHereMetadata {
  page?: number;
}
...
type MangaHereImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class MangaHereExtension implements MangaHereImplementation {
  ...
  async getAdvancedSearchForm(
    ...
    const meta = query.metadata as
      | { searchMeta?: MangaHereSearchMeta }
      | undefined;
  ...
  async getDiscoverSectionItems(
    ...
    const meta = metadata as MangaHereMetadata | undefined;
    ...
    $(".manga-list-1-list li").each((_, element) => {
      ...
      const imageUrl = this.absoluteUrl(
        el.find("img.manga-list-1-cover").first().attr("src") || "",
      );
      items.push({
        type:
          section.id === "latest"
  ...
  async getSearchResults(
    ...
    const searchMeta = (
      query.metadata as { searchMeta?: MangaHereSearchMeta } | undefined
    )?.searchMeta;
    ...
    $(".manga-list-4-list > li").each((_, element) => {
      ...
      if (!href || !title) return;
      const imageUrl = this.absoluteUrl(
        el.find("img.manga-list-4-cover").first().attr("src") || "",
      );
      results.push({
        mangaId: this.parsePath(href),
        imageUrl,
  ...
  private buildSearchUrl(
    titleQuery: string,
    searchMeta: MangaHereSearchMeta | undefined,
    page: number,
  ): string {
    let url = `${BASE_URL}/search?page=${page}&title=${encodeURIComponent(
      titleQuery,
    )}&stype=1`;
    ...
    if (searchMeta) {
      ...
      if (completion) url += `&st=${completion}`;
      const rating = searchMeta.rating?.[0];
      if (rating) url += `&rating_method=gt&rating=${rating}`;
      const include = searchMeta.includeGenres ?? [];
      if (include.length > 0) url += `&genres=${include.join(",")}`;
      const exclude = searchMeta.excludeGenres ?? [];
      if (exclude.length > 0) url += `&nogenres=${exclude.join(",")}`;
      if (searchMeta.artist)
        url += `&artist_method=cw&artist=${encodeURIComponent(
          searchMeta.artist,
        )}`;
      if (searchMeta.author)
        url += `&author_method=cw&author=${encodeURIComponent(
          searchMeta.author,
        )}`;
      if (searchMeta.year)
        url += `&released_method=eq&released=${encodeURIComponent(
          searchMeta.year,
        )}`;
    }
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const title =
      $(".detail-info-right-title-font").first().text().trim() ||
      this.safeDecode(mangaId);
    const thumbnailUrl = this.absoluteUrl(
      $("img.detail-info-cover-img").first().attr("src") || "",
    );
  ...
  private findPackedScript($: CheerioAPI): string | undefined {
  ...
  private unpack(packed: string): string {
    const argsMatch = packed.match(
      /\}\s*\(\s*'([\s\S]*)'\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*'([\s\S]*?)'\s*\.split\(\s*'\|'\s*\)/,
    );
    ...
    for (let i = count - 1; i >= 0; i--) {
      ...
      if (word) {
        ...
        payload = payload.replace(
          new RegExp("\\b" + this.escapeRegex(token) + "\\b", "g"),
          word,
        );
      }
    }
  ...
  private async fetchPagesByKey(
    ...
    const pagesNumber = parseInt(
      $(pagerLinks[total - 2]).attr("data-page") || "0",
      10,
    );
    if (!pagesNumber) return pages;
  ...
  private between(source: string, startToken: string, endToken: string): string {
