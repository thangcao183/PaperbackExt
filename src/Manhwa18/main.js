...
import { Manhwa18SearchForm } from "./forms";
...
export class Manhwa18Extension {
    ...
    async getMangaDetails(mangaId) {
        ...
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const title = $(".series-name a, .au-bento h1, .au-crumb a:last-child").first().text().trim() || this.safeDecode(mangaId);
        const thumbnailUrl = this.styleBgUrl($(".series-cover .img-in-ratio").first().attr("style"));
