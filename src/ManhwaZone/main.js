...
export class ManhwaZoneExtension {
    ...
    async getSearchResults(query, metadata) {
        ...
        if (titleQuery.startsWith("https://") && titleQuery.includes("manhwazone.com")) {
            ...
            if (detail)
                return { items: [detail], metadata: undefined };
    ...
    parseMangaList($) {
        ...
        const hasNextPage = $("a[rel=next], nav a:contains(›)").length > 0 || items.length >= 24;
        return { items, hasNextPage };
    }
    parseDetailAsSearch($, url) {
        const title = $("h1.page-title").first().text().trim();
        if (!title)
            return undefined;
        const imageUrl = this.imageFromElement($("img.aspect-\\[7\\/10\\], figure.relative img").first());
        return {
    ...
    async getMangaDetails(mangaId) {
        ...
        const thumbnailUrl = this.imageFromElement($("img.aspect-\\[7\\/10\\], figure.relative img").first());
        const genres = $("a.badge-genre")
            .map((_, el) => $(el).text().trim())
            .get()
            .filter((g) => g.length > 0);
        const statusText = $("span.badge-sm, span:contains(On Going), span:contains(Completed)")
            .first()
            .text()
            .trim();
        let author;
        ...
        if (jsonLd) {
            const m = jsonLd.match(/"author":\s*\[\s*\{"@type":"Person","name":"([^"]+)"/);
    ...
    async getChapters(sourceManga) {
        ...
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const wireDiv = $("div[wire\\:snapshot][wire\\:id][wire\\:init=bootLoad]").first();
        const snapshot = wireDiv.attr("wire:snapshot");
        if (!snapshot)
            return [];
        const csrfToken = $("meta[name=csrf-token]").first().attr("content") || "";
        const payload = {
            _token: csrfToken,
            components: [
                {
        ...
        const chapters = [];
        try {
            const updateJson = JSON.parse(Application.arrayBufferToUTF8String(data));
            const snapshotStr = updateJson.components?.[0]?.snapshot;
            if (!snapshotStr)
                return [];
            const snapshotJson = JSON.parse(snapshotStr);
                ...
                try {
                    ...
                    if (conf.p &&
                        ...
                        for (let i = 1; i <= conf.tt; i++) {
                            ...
                            pages.push(`https://img.mangalaxy.net/_img/${conf.p}/${pageStr}.webp?e=${conf.expire}&s=${conf.signature}`);
                        }
                    }
                }
                catch {
