...
export class MgreadIoExtension {
    ...
    async getDiscoverSectionItems(section, metadata) {
        ...
        $(MANGA_GRID_SELECTOR).each((_, element) => {
            ...
            if (!parsed)
                return;
            if (this.isAnimeEntry(parsed.title, parsed.mangaId))
                return;
            if (seen.has(parsed.mangaId))
                return;
            seen.add(parsed.mangaId);
            items.push({
                type: section.id === "manga-ranking"
    ...
    async getSearchResults(query, metadata) {
        ...
        if (titleQuery !== "") {
            const url = `${BASE_URL}/wp-json/initlise/v1/search?term=${encodeURIComponent(titleQuery)}&page=${page}`;
            ...
            for (const dto of json ?? []) {
                ...
                if (this.isAnimeEntry(parsedTitle, mangaId))
                    continue;
                seen.add(mangaId);
                results.push({
                    mangaId,
        ...
        const segments = ["advanced-filter"];
        if (page > 1) {
    ...
    async getMangaDetails(mangaId) {
        ...
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const title = $("#manga-title").first().clone().children().remove().end().text().trim() ||
            ($("meta[property=og:title]").first().attr("content") || "")
                .split(" [Ch.")[0]
                .trim() ||
            this.safeDecode(mangaId);
        const thumbnailUrl = this.imageFromElement($(".story-cover img, meta[property=og:image]").first());
    ...
    async getChapters(sourceManga) {
        ...
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const idAttr = $("#manga-title[data-id]").first().attr("data-id") ||
            $("#chapter-search-input[data-manga-id]").first().attr("data-manga-id") ||
            "";
        const mangaApiId = parseInt(idAttr, 10);
        if (!Number.isNaN(mangaApiId)) {
            const restChapters = await this.fetchChapterList(mangaApiId, mangaPath, sourceManga);
            if (restChapters.length > 0)
                return restChapters;
        }
        // HTML fallback.
        const chapters = [];
    ...
    async fetchChapterList(mangaApiId, mangaPath, sourceManga) {
        const chapters = [];
        ...
        do {
            ...
            let listDto;
            try {
                listDto = await this.fetchJson({ url, method: "GET" });
            }
            catch {
                break;
            }
            ...
            for (const item of listDto.items ?? []) {
                ...
                const chapterName = num % 1 === 0 ? String(Math.trunc(num)) : String(num);
