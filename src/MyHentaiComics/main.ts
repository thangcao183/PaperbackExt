...
interface MyHentaiComicsMetadata {
  page?: number;
}

interface MyHentaiComicsSearchMetadata {
  category?: string;
}
...
type MyHentaiComicsImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class MyHentaiComicsExtension implements MyHentaiComicsImplementation {
  ...
  async getDiscoverSectionItems(
    ...
    if (section.id === "categories") {
      const items: DiscoverSectionItem[] = CATEGORIES.map((cat) => ({
        ...
        searchQuery: {
          title: "",
          metadata: { category: cat.id } satisfies MyHentaiComicsSearchMetadata,
        },
        name: cat.name,
        metadata: undefined,
      }));
    ...
    const meta = metadata as MyHentaiComicsMetadata | undefined;
  ...
  async getSearchResults(
    ...
    const searchMeta = query.metadata as
      | MyHentaiComicsSearchMetadata
      | undefined;
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const thumbnailUrl = this.imageFromElement(
      $("div.comic-cover img").first(),
    );

    const synopsisLines: string[] = [];
    if (artists.length > 0) synopsisLines.push(`Artists: ${artists.join(", ")}`);
    if (groups.length > 0) synopsisLines.push(`Groups: ${groups.join(", ")}`);
    if (pagesText) synopsisLines.push(pagesText);
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    const imageUrl =
      $("ul.gallery-slide li img").first().attr("src") || "";
    if (!imageUrl) {
  ...
  private encodeSpaces(url: string): string {
