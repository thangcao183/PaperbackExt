...
import { NineAnimeSearchForm } from "./forms";
...
export class NineAnimeExtension {
    ...
    async getSearchResults(query, metadata) {
        ...
        let url;
        if (titleQuery) {
            url = `${BASE_URL}/search/?name=${encodeURIComponent(titleQuery)}&page=${page}.html`;
        }
        else {
    ...
    async getMangaDetails(mangaId) {
        ...
        const thumbnailUrl = this.imageFromElement(top.find("img.detail-cover").first());
        const author = top
    ...
    async tryExternalPages(finalUrl) {
        try {
            const cid = finalUrl
                .split("-10-1")[0]
                .split("?")[0]
            ...
            const jumpUrl = this.absoluteUrl($iframe("a.vision-button").first().attr("href") || "");
            if (!jumpUrl)
                return [];
