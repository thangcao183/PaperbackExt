...
interface NineHentaiMetadata {
  page?: number;
  sort?: number;
}

// Shape of a single book as returned by the JSON API.
interface ApiManga {
  id: number;
  total_page: number;
  title: string;
...
interface SingleMangaApiResponse {
  results: ApiManga;
}
...
type NineHentaiImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class NineHentaiExtension implements NineHentaiImplementation {
  ...
  async getDiscoverSectionItems(
    ...
    const meta = metadata as NineHentaiMetadata | undefined;
  ...
  async getSearchResults(
    ...
    if (titleQuery.startsWith("id:")) {
      ...
      if (isNaN(id)) return { items: [], metadata: undefined };
      const manga = await this.fetchSingleManga(id);
      if (!manga) return { items: [], metadata: undefined };
      return {
        items: [
          {
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const thumbnailUrl = this.absoluteUrl(
      info.find("div#cover v-lazy-image").first().attr("src") || "",
    );
    const artist = this.selectText($, info, "div.field-name:contains(Artist:) a.tag");
    const author =
      this.selectText($, info, "div.field-name:contains(Group:) a.tag") ||
      "Unknown circle";
    ...
    const parody = this.selectText(
      $,
      info,
      "div.field-name:contains(Parody:) a.tag",
    );
    if (parody) descriptionParts.push(`Parody: ${parody}`);
    const category = this.selectText(
      $,
      info,
      "div.field-name:contains(Category:) a.tag",
    );
    if (category) descriptionParts.push(`Category: ${category}`);
    const language = this.selectText(
      $,
      info,
      "div.field-name:contains(Language:) a.tag",
    );
    if (language) descriptionParts.push(`Language: ${language}`);
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    try {
      ...
      if (response.status === 404) totalPages--;
    } catch {
  ...
  private async fetchSearch(opts: {
    page: number;
    sort: number;
    text?: string;
  }): Promise<{ mangas: ApiManga[]; hasNext: boolean }> {
    const payload = {
      search: {
        text: opts.text ?? "",
    ...
    const parsed = this.parseJson<SearchApiResponse>(data);
    const mangas = parsed?.results ?? [];
    const totalCount = parsed?.total_count ?? 0;
    return { mangas, hasNext: totalCount > opts.page };
  }

  private async fetchSingleManga(id: number): Promise<ApiManga | undefined> {
    const [response, data] = await Application.scheduleRequest({
    ...
    const parsed = this.parseJson<SingleMangaApiResponse>(data);
  ...
  private thumbnailUrl(m: ApiManga): string {
    return `${m.image_server}${m.id}/cover-small.jpg`;
  }

  private getImageUrl(m: ApiManga): string {
  ...
  private mangaUrl(mangaId: string): string {
    ...
    if (/^\d+$/.test(slug)) return `${BASE_URL}/g/${slug}`;
  ...
  private parseChapterDate(date: string): Date {
    ...
    if (isNaN(value)) return new Date(0);
