...
interface NixMangaMetadata {
  page?: number;
}

// Minimal structural type for the WebCrypto subtle API used for SHA-256.
interface SubtleLike {
  digest(algorithm: string, data: ArrayBuffer): Promise<ArrayBuffer>;
}

// ----------------------------------------------------------------
// JSON API shapes (mirrors upstream Dto.kt)
// ----------------------------------------------------------------
...
interface ChapterApi {
  ...
  scanlator?: ScanlatorApi | null;
}

interface PaginatedChaptersApi {
  chapters?: ChapterApi[];
  page?: number;
  total_pages?: number;
}
...
type NixMangaImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class NixMangaExtension implements NixMangaImplementation {
  ...
  async getSearchResults(
    ...
    const results: SearchResultItem[] = comics.map((comic) => ({
      mangaId: this.toSafeId(comic.slug),
      imageUrl: comic.cover ?? "",
      title: comic.title,
      subtitle: undefined,
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const dto = await this.fetchApi<ComicApi>(`/comics/slug/${slug}`);
  ...
  async getChapters(sourceManga: SourceManga): Promise<Chapter[]> {
    ...
    for (;;) {
      ...
      const dto = await this.fetchApi<PaginatedChaptersApi>(path);
      ...
      if (current >= totalPages || list.length === 0) break;
      page++;
    }
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    const dto = await this.fetchApi<PagesApi>(`/chapters/${id}?skip_view=true`);
  ...
  private async refreshAuthValues(): Promise<void> {
    ...
    const rJoin = (arr: string[]): string =>
      arr.map((s) => reverse(s)).join("");
  ...
  private async computeSignature(endpoint: string): Promise<string> {
  ...
  private buildChapterName(ch: ChapterApi): string {
    let name: string;
  ...
  private hasNextComicsPage(dto: PaginatedComicsApi, page: number): boolean {
    const totalPages = dto.total_pages ?? 1;
  ...
  private utf8Buffer(str: string): ArrayBuffer {
    const out: number[] = [];
