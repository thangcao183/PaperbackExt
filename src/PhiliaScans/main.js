...
class PhiliaScansInterceptor extends PaperbackInterceptor {
    ...
    async interceptResponse(request, response, data) {
        ...
        if (!SCRAMBLED_PATH.test(lastSegment))
            return data;
        try {
...
export class PhiliaScansExtension {
    ...
    async getDiscoverSectionItems(section, metadata) {
        if (section.id === "genres") {
            const items = GENRES.map(([name, slug]) => ({
    ...
    async getMangaDetails(mangaId) {
        ...
        const altTitles = (result.alternativeTitles ?? []).filter((t) => !!t && t.trim().length > 0);
    ...
    async getChapters(sourceManga) {
        ...
        for (const item of result.items ?? []) {
            ...
            const isLocked = item.purchased === false && (item.coinPrice ?? 0) !== 0;
            const lock = isLocked ? "🔒 " : "";
            const rawTitle = (item.title || "").trim();
            const validTitle = rawTitle && rawTitle !== "null" && rawTitle !== number
                ? rawTitle
                : "";
            const name = lock +
                (validTitle
    ...
    async getChapterDetails(chapter) {
        ...
        if (result.hasAccess === false) {
            throw new Error("Log in via WebView and purchase this chapter to read it.");
        }
        const rawPages = result.chapter?.pages ?? [];
        const sorted = [...rawPages].sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
        ...
        if (chapterId !== undefined && chapterId !== null) {
            try {
                ...
                if (token)
                    readerHeaders["x-reader-access-token"] = token;
                const pageKeys = await this.fetchJson({
                ...
                if (pageKeys.sessionDefault === true) {
                    ...
                    if (sessionId) {
                        try {
                            const drm = await this.fetchJson({
...
async function decodePage(fragment, data) {
    ...
    const usePayload = payloadA !== "null" &&
        payloadA.length > 0 &&
        payloadB !== "null" &&
        payloadB.length > 0;
    const chapterKeyB64ForInject = usePayload ? "" : chapterKeyB64;
    if (!usePayload && !chapterKeyB64)
        return data;
