...
const RELATIVE_DATE_REGEX = /(\d+)\s+(second|minute|hour|day|week|month|year)s?\s+ago/;
...
export class RoliaScanExtension {
    ...
    async getDiscoverSectionItems(section, metadata) {
        if (section.id === "genres") {
            const items = ROLIA_TAGS.map(([name, id]) => ({
    ...
    async getMangaDetails(mangaId) {
        ...
        const url = `${BASE_URL}/wp-json/wp/v2/manga/${encodeURIComponent(id)}?_embed`;
        const data = await this.fetchJson({
            url,
            method: "GET",
        });
        const title = this.unescapeHtml(data.title?.rendered ?? "") || slug;
        const synopsis = this.unescapeHtml(this.stripHtml(data.content?.rendered ?? ""));
        const embedded = data._embedded ?? {};
        const thumbnailUrl = embedded["wp:featuredmedia"]?.[0]?.source_url ?? "";
        const author = this.getTerms(embedded, "manga_author").join(", ");
        const genres = new Set(this.getTerms(embedded, "post_tag"));
        if (!["Manhwa", "Manhua", "Manga"].some((t) => genres.has(t)) &&
            data.type) {
    ...
    async getChapters(sourceManga) {
        ...
        const token = this.md5(`${timestamp}mng_ch_${this.isoDate(new Date())}`).substring(0, 16);
        const url = `${BASE_URL}/auth/manga-chapters?` +
        ...
        for (const ch of data.chapters ?? []) {
            if (!ch.language || ch.language.toLowerCase() !== LANG)
                continue;
    ...
    async getChapterDetails(chapter) {
        ...
        const url = `${BASE_URL}/auth/chapter-content?chapter_id=${encodeURIComponent(chapterId)}`;
    ...
    // Browse API
    // ----------------------------------------------------------------
    async loadBrowsePage(page, opts) {
        ...
        for (let apiPage = startApiPage; apiPage <= endApiPage; apiPage++) {
            const body = JSON.stringify({
                ...
                years: this.stringifyList([]),
                genres: this.stringifyList(opts.genres ?? []),
                types: this.stringifyList([]),
                statuses: this.stringifyList([]),
                sort: opts.sort,
                genreMatchMode: "any",
            });
            const data = await this.fetchJson({
    ...
    stringifyList(values) {
        return JSON.stringify(values);
    }
    // ----------------------------------------------------------------
    // Id helpers
    // ----------------------------------------------------------------
    buildMangaId(id, slug) {
