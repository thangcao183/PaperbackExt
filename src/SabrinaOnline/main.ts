...
const THUMBNAIL =
  "https://dummyimage.com/768x994/000/ffffff.jpg&text=Sabrina%20Online";
...
type SabrinaOnlineImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class SabrinaOnlineExtension implements SabrinaOnlineImplementation {
      ...
      row.find("td").each((index, td) => {
        $(td)
          ...
          .each((_a, a) => {
            ...
            const hasYear =
              !!section && section.length > 0 && /\d/.test(section.charAt(0));
            entries.push({
              chapterId,
  ...
  async getChapterDetails(chapter: Chapter): Promise<ChapterDetails> {
    ...
    $("center img").each((_, element) => {
      ...
      if (!src) return;
      if (!src.includes("strips/") && !src.includes("pages/")) return;
