...
function feScalarMult(q, n, p) {
    ...
    const x = feNew();
    feUnpack(x, p);
    const a = feNew(), b = feNew(), c = feNew(), d = feNew(), e = feNew(), f = feNew();
    feSet(b, x);
    a[0] = 1;
    d[0] = 1;
    for (let i = 254; i >= 0; --i) {
...
export function x25519ScalarMult(privateKey, publicKey) {
...
export async function hmacSha256Hex(key, message) {
    ...
    const cryptoKey = await subtle.importKey("raw", keyBuf, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
...
function store32LE(dst, offset, w) {
    dst[offset] = w & 0xff;
...
function chachaBlock(output, input) {
...
function chacha20StreamIETF(c, clen, nonce, key) {
    ...
    while (pos < clen) {
        ...
        for (let i = 0; i < 16; i++)
            store32LE(blockBytes, i * 4, output[i]);
...
function chacha20StreamIETFXorIC(c, m, mlen, nonce, ic, key) {
    const input = new Int32Array(16);
    const output = new Int32Array(16);
    const blockBytes = new Uint8Array(64);
    input[0] = 0x61707865;
...
function poly1305Blocks(state, m, offset, bytes) {
...
function poly1305Update(state, m, offset, mlen) {
    let pos = offset;
    let remaining = mlen;
    if (state.leftover > 0) {
...
function poly1305Finalize(state, mac) {
...
function secretStreamInitPull(header, key) {
...
function store64LE(dst, offset, value) {
...
function secretStreamPull(state, inp, inlen) {
    if (inlen < ABYTES)
        return null;
 ...
 */
export function decryptSecretStream(streamKey, payload) {
    if (payload.length < PREFIX_LENGTH + STREAM_HEADER_LENGTH)
        return null;
    const header = payload.subarray(PREFIX_LENGTH, PREFIX_LENGTH + STREAM_HEADER_LENGTH);
