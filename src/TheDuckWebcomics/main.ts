...
import {
  TheDuckWebcomicsSearchForm,
  TheDuckWebcomicsSearchMeta,
} from "./forms";

const BASE_URL = "https://www.theduckwebcomics.com";

interface TheDuckWebcomicsEntry {
  mangaId: string;
  title: string;
  imageUrl: string;
  author: string;
...
type TheDuckWebcomicsImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class TheDuckWebcomicsExtension
  implements TheDuckWebcomicsImplementation
{
  ...
  private detailsCache: Map<string, TheDuckWebcomicsEntry> = new Map();
  ...
  async getSearchResults(
    ...
    const searchMeta = (query.metadata as { searchMeta?: TheDuckWebcomicsSearchMeta })
      ?.searchMeta;
  ...
  private buildSearchUrl(
    titleQuery: string,
    page: number,
    searchMeta?: TheDuckWebcomicsSearchMeta,
  ): string {
    const params: string[] = [];
    ...
    if (searchMeta) {
      ...
      if (lastUpdate) params.push(`last_update=${encodeURIComponent(lastUpdate)}`);
    }
  ...
  private async fetchList(
    url: string,
  ): Promise<{ entries: TheDuckWebcomicsEntry[]; hasNextPage: boolean }> {
    const $ = await this.fetchCheerio({ url, method: "GET" });
    const entries: TheDuckWebcomicsEntry[] = [];
  ...
  ): TheDuckWebcomicsEntry | undefined {
