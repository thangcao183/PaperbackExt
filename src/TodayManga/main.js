...
export class TodayMangaExtension {
    ...
    async getMangaDetails(mangaId) {
        ...
        const title = serie.find("h1").first().text().trim() || this.safeDecode(mangaId);
    ...
    async getChapterDetails(chapter) {
        ...
        $(".chapter-content > img[data-index]").each((_, element) => {
            ...
            if (src)
                collected.push({ index: isNaN(idx) ? 0 : idx, url: src });
        });
    ...
    parseRelativeDate(text) {
        ...
        if (lower.includes("second"))
            return new Date(d - amount * sec);
        if (lower.includes("minute"))
            return new Date(d - amount * min);
        if (lower.includes("hour"))
            return new Date(d - amount * hour);
        if (lower.includes("day"))
            return new Date(d - amount * day);
        if (lower.includes("week"))
            return new Date(d - amount * 7 * day);
        if (lower.includes("month"))
            return new Date(d - amount * 30 * day);
        if (lower.includes("year"))
            return new Date(d - amount * 365 * day);
        return new Date(0);
    }
    ...
    parseStatus(status) {
        ...
        if (s === "complete" || s.includes("complete"))
            return "Completed";
