...
class WebNovelInterceptor extends PaperbackInterceptor {
    ...
    async interceptRequest(request) {
        ...
        if (this.csrfToken &&
            ...
            request.url = `${request.url}${sep}${CSRF_TOKEN_NAME}=${encodeURIComponent(this.csrfToken)}`;
        }
        return request;
    }
...
export class WebNovelExtension {
    ...
    async getDiscoverSectionItems(section, metadata) {
        if (section.id === "genres") {
            const items = GENRES.filter((g) => g.id !== "0").map((g) => ({
        ...
        const items = parsed.items.map((m) => isFeatured
            ? {
    ...
    async getSearchResults(query, metadata) {
        ...
        if (titleQuery !== "") {
            const url = `${BASE_API_URL}${QUERY_SEARCH_PATH}?type=manga&pageIndex=${page}&keywords=${encodeURIComponent(titleQuery)}`;
            const json = await this.fetchJson(url);
    ...
    async getMangaDetails(mangaId) {
        ...
        const url = `${BASE_API_URL}/comic/getComicDetailPage?comicId=${encodeURIComponent(comicId)}`;
        const json = await this.fetchJson(url);
        const comic = this.asRecord(this.asRecord(json)["comicInfo"]);
        const title = this.asString(comic["comicName"]) || comic["comicId"]?.toString() || comicId;
    ...
    async getChapters(sourceManga) {
        ...
        const url = `${BASE_API_URL}/comic/getChapterList?comicId=${encodeURIComponent(comicId)}`;
        const json = await this.fetchJson(url);
    ...
    async getChapterDetails(chapter) {
        ...
        const url = `${BASE_API_URL}/comic/getContent?comicId=${encodeURIComponent(comicId)}&chapterId=${encodeURIComponent(chapterId)}&width=9999`;
        const json = await this.fetchJson(url);
    ...
    // URL / id helpers
    // ----------------------------------------------------------------
    filterSearchUrl(page, genre, bookStatus, sort) {
        return (`${BASE_API_URL}${FILTER_SEARCH_PATH}?categoryType=2&pageIndex=${page}` +
    ...
    parseChapterId(chapterId) {
        ...
        if (idx < 0)
            return { comicId: decoded, chapterId: "" };
        return {
