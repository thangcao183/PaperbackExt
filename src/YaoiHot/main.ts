...
type YaoiHotImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class YaoiHotExtension implements YaoiHotImplementation {
  ...
  async getDiscoverSectionItems(
    ...
    const meta = metadata as YaoiHotMetadata | undefined;
  ...
  async getMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const title =
      $(".manga-title").first().text().trim() || this.safeDecode(mangaId);
    const authorLine = $(".author-line").first().text().trim();
    const author =
      authorLine.indexOf("Author:") >= 0
  ...
  private listUrl(orderby: string, page: number): string {
