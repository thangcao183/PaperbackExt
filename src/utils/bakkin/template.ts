...
// --- JSON DTO shapes (Bakkin main.php blob) ---

interface BakkinChapter {
  dir: string;
  name?: string | null;
  pages: string[];
}

interface BakkinVolume {
  dir: string;
  name?: string | null;
  chapters: BakkinChapter[];
}

interface BakkinSeries {
  dir: string;
  name?: string | null;
  author?: string | null;
  status?: string | null;
  thumb?: string | null;
  volumes: BakkinVolume[];
}
...
type BakkinImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class BakkinExtension implements BakkinImplementation {
  ...
  private seriesCache: BakkinSeries[] | undefined;
  ...
  requestManager: BakkinInterceptor;
  ...
  constructor(config: BakkinConfig) {
  ...
  private async fetchCatalog(): Promise<BakkinSeries[]> {
    ...
    const blob = await this.fetchJson<Record<string, BakkinSeries>>({
      url,
      method: "GET",
    });
    const series = Object.values(blob ?? {}).filter(
      (s) => s && typeof s.dir === "string",
    );
    this.seriesCache = series;
    return series;
  }

  private seriesTitle(series: BakkinSeries): string {
    const name = (series.name ?? "").trim();
    return name.length > 0 ? name : series.dir;
  }

  private nodeTitle(node: { dir: string; name?: string | null }): string {
    const name = (node.name ?? "").trim();
    return name.length > 0 ? name : node.dir;
  }

  private coverUrl(series: BakkinSeries): string {
  ...
  async getDiscoverSectionItems(
    ...
    if (section.id !== "all_titles") return { items: [] };
  ...
  async getSearchResults(
    ...
    const filtered =
      titleQuery.length > 0
        ? series.filter((s) =>
            this.seriesTitle(s).toLowerCase().includes(titleQuery),
          )
        : series;

    const results: SearchResultItem[] = filtered.map((s) => ({
      mangaId: this.toSafeId(s.dir),
      imageUrl: this.coverUrl(s),
      title: this.seriesTitle(s),
      subtitle: undefined,
  ...
  private parseChapterNumber(chapterDir: string): number {
