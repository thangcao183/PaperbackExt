...
export async function loadImageFromBuffer(data, mimeType) {
...
export function decodeDataUrlToArrayBuffer(dataUrl) {
    const comma = dataUrl.indexOf(",");
    if (comma < 0)
        throw new Error("toDataURL returned malformed data URL");
 ...
 */
function cropBlit(ctx, src, srcW, srcH, sx, sy, dx, dy, cw, ch) {
    if (cw <= 0 || ch <= 0)
        return;
 ...
 */
export async function descrambleMangago(data, mimeType, keyArr, cols) {
    if (cols <= 0)
        return data;
    const tileCount = cols * cols;
    if (keyArr.length < tileCount - 1)
        return data;
 ...
 */
export async function remapTilesByLookup(data, mimeType, cols, rows, lookup, outputMimeType = mimeType) {
    ...
    for (let i = 0; i < lookup.length; i++) {
        ...
        cropBlit(ctx, src, width, height, srcCol * tw, srcRow * th, dstCol * tw, dstRow * th, tw, th);
    }
 ...
 */
export async function descrambleViz(data, mimeType, key, metaW, metaH) {
    ...
    const blit = (sx, sy, dx, dy, w, h) => {
        ...
        if (dy + ch > newHeight)
            ch = newHeight - dy;
        if (sx < 0 || sy < 0 || dx < 0 || dy < 0 || cw <= 0 || ch <= 0)
            return;
    ...
    // Left border.
    blit(0, blockHeight + GUTTER, 0, blockHeight, blockWidth, newHeight - 2 * blockHeight);
    // Bottom border.
    blit(0, (CELL_HEIGHT_COUNT - 1) * (blockHeight + GUTTER), 0, (CELL_HEIGHT_COUNT - 1) * blockHeight, newWidth, height - (CELL_HEIGHT_COUNT - 1) * (blockHeight + GUTTER));
    // Right border.
    blit((CELL_WIDTH_COUNT - 1) * (blockWidth + GUTTER), blockHeight + GUTTER, (CELL_WIDTH_COUNT - 1) * blockWidth, blockHeight, blockWidth + (newWidth - CELL_WIDTH_COUNT * blockWidth), newHeight - 2 * blockHeight);
    // Inner cells: source cell m -> destination cell key[m].
    for (let m = 0; m < key.length; m++) {
        const y = key[m];
        blit(((m % INNER_CELL_COUNT) + 1) * (blockWidth + GUTTER), (Math.floor(m / INNER_CELL_COUNT) + 1) * (blockHeight + GUTTER), ((y % INNER_CELL_COUNT) + 1) * blockWidth, (Math.floor(y / INNER_CELL_COUNT) + 1) * blockHeight, blockWidth, blockHeight);
    }
 ...
 */
export async function remapKMangaCells(data, mimeType, sourceOrder) {
    ...
    for (let i = 0; i < 16; i++) {
        ...
        const dstCol = i % 4;
        const dstRow = (i / 4) | 0;
        cropBlit(ctx, src, width, height, srcCol * blockWidth, srcRow * blockHeight, dstCol * blockWidth, dstRow * blockHeight, blockWidth, blockHeight);
    }
    return decodeDataUrlToArrayBuffer(canvas.toDataURL(mimeType));
}
