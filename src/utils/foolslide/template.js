...
export class FoolSlideExtension {
    sourceName;
    defaultBaseUrl;
    urlModifier;
    contentRating;
    ...
    async getDiscoverSectionItems(section, metadata) {
        ...
        $("div.group").each((_, element) => {
            ...
            if (img.length > 0) {
                imageUrl = this.resolveUrl(img.attr("src") ?? "").replace("/thumb_", "/");
            }
            parsed.push({
                mangaId,
        ...
        const hasNext = $("div.next").length > 0 && page < FoolSlideExtension.MAX_SEARCH_PAGES;
        return {
            items,
    ...
    async fetchDirectoryCovers(needed) {
        ...
        try {
            for (let page = 1; page <= FoolSlideExtension.MAX_SEARCH_PAGES; page++) {
                ...
                $("div.group").each((_, element) => {
                    ...
                    const imageUrl = this.resolveUrl(img.attr("src") ?? "").replace("/thumb_", "/");
                    if (imageUrl) {
    ...
    async getMangaDetails(mangaId) {
        ...
        const author = labelled.author ??
            this.matchInfo(infoHtml, /(?:Author|Autore)<\/b>:\s?([^\n<]*)[\n<]/i);
        const artist = labelled.artist ?? this.matchInfo(infoHtml, /Artist<\/b>:\s?([^\n<]*)[\n<]/i);
        const description = labelled.description ??
            this.matchInfo(infoHtml, /(?:Synopsis|Description|Trama)<\/b>:\s?([^\n<]*)[\n<]/i);
        let thumbnailUrl = "";
    ...
    async getChapters(sourceManga) {
        ...
        const $ = await this.fetchCheerioPost(url, `adult=${showAdult}`);
        const chapters = [];
        $("div.group div.element, div.list div.element").each((_, element) => {
            const el = $(element);
    ...
    async thumbnailFromFirstChapter($) {
    ...
    parseDate(text) {
        ...
        const relative = cleaned.match(/(\d+)\s*(sec|second|min|minute|hour|hr|day|week|wk|month|year|yr)/i);
        if (relative) {
    ...
    unitToMillis(unit) {
        ...
        if (unit.startsWith("week") || unit === "wk")
