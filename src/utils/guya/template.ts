...
interface GuyaSearchMetadata {
  page?: number;
}

interface GuyaChapter {
  title?: string;
  folder?: string;
...
type GuyaImplementation = Extension &
  SearchResultsProviding &
  MangaProviding &
  ChapterProviding &
  CloudflareBypassRequestProviding &
...
export class GuyaExtension implements GuyaImplementation {
  ...
  requestManager: GuyaInterceptor;
  ...
  async getDiscoverSectionItems(
    ...
    if (section.id === "latest_section") {
      entries = entries.sort(
        (a, b) => (b[1].last_updated ?? 0) - (a[1].last_updated ?? 0),
      );
    }
    ...
    const items: DiscoverSectionItem[] = entries.map(([title, series]) => ({
      type: itemType,
    ...
    if (chapterData) {
      ...
      for (const file of files) {
        if (typeof file === "string" && file.startsWith("http")) {
          ...
          pages.push(
            `${this.baseUrl}/media/manga/${slug}/chapters/${folder}/${groupId}/${file}`,
          );
        }
      }
    }
  ...
  private async fetchAllSeries(): Promise<Record<string, GuyaSeries>> {
    const url = new URLBuilder(this.baseUrl)
      .addPath("api")
      .addPath("get_all_series")
      .build();
    return this.fetchJson<Record<string, GuyaSeries>>({ url, method: "GET" });
  }

  private async fetchSeries(slug: string): Promise<GuyaSeries> {
    const url = new URLBuilder(this.baseUrl)
      .addPath("api")
      .addPath("series")
      .addPath(slug)
      .build();
    return this.fetchJson<GuyaSeries>({ url, method: "GET" });
  }

  private pickGroup(chapter: GuyaChapter): string {
    if (chapter.preferred_sort && chapter.preferred_sort.length > 0) {
      const preferred = chapter.preferred_sort.find(
        (g) => chapter.groups && chapter.groups[g],
      );
