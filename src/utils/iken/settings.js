...
function useChaptersApiKey(sourceName) {
...
/**
 * Whether locked (inaccessible) chapters should be shown. Defaults to false.
 */
 ...
 */
export function getUseChaptersApi(sourceName) {
    const value = Application.getState(useChaptersApiKey(sourceName));
    return typeof value === "boolean" ? value : false;
}
export function setUseChaptersApi(sourceName, value) {
    Application.setState(value, useChaptersApiKey(sourceName));
}
/**
 * Settings form for an Iken source. Lets the user override the base URL
 * (useful when a site changes domain) and toggle locked-chapter visibility.
 */
export class IkenSettingsForm extends Form {
    sourceName;
    defaultBaseUrl;
    override;
    showLocked;
    ...
    getSections() {
        ...
        return [
            ...
            Section({
                id: "chapters",
                footer: "Show locked/inaccessible chapters in the chapter list. They are " +
                    "marked with a lock icon and may not be readable.",
            }, [
