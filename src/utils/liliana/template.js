...
export class LilianaExtension {
    ...
    async getSearchResults(query, metadata) {
        ...
        let url;
        if (titleQuery) {
            url = `${this.baseUrl}/search/${page}/?keyword=${encodeURIComponent(titleQuery)}`;
        }
        else {
        ...
        const hasNextPage = $(".blog-pager > span.pagecurrent + span").length > 0;
    ...
    async getMangaDetails(mangaId) {
        ...
        let author = $("div.y6x11p i.fas.fa-user + span.dt")
            .first()
            .text()
            .trim();
    ...
    async getChapters(sourceManga) {
        ...
        elements.forEach((element, index) => {
            ...
            if (dt) {
                ...
                if (!isNaN(epoch))
                    publishDate = new Date(epoch * 1000);
            }
            let chapNum = -1;
