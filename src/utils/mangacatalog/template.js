...
export class MangaCatalogExtension {
    sourceName;
    defaultBaseUrl;
    contentRating;
    langCode;
    ...
    constructor(config) {
        ...
        this.requestManager = new MangaCatalogInterceptor("main", () => this.baseUrl);
    }
    ...
    async getDiscoverSectionItems(_section, _metadata) {
        const items = await Promise.all(this.mangaList.map(async (entry) => {
    ...
    async getSearchResults(query, _metadata) {
        ...
        const matches = this.mangaList.filter((entry) => !titleQuery || entry.title.toLowerCase().includes(titleQuery));
        const items = await Promise.all(matches.map(async (entry) => {
            const mangaId = this.pathToId(entry.url);
            return {
                mangaId,
                imageUrl: await this.resolveCover(mangaId),
    ...
    async getMangaDetails(mangaId) {
        ...
        if (thumbnail)
            this.setCachedCover(mangaId, thumbnail);
        return {
            mangaId,
            mangaInfo: {
