...
import { MangaThemesiaSearchForm, } from "./forms";
import { getBaseUrlOverride, MangaThemesiaSettingsForm, } from "./settings";
...
export class MangaThemesiaExtension {
    ...
    discoverItemSelectorOverride;
    seriesDetailsSelectorOverride;
    seriesTitleSelectorOverride;
    seriesThumbnailSelectorOverride;
    seriesDescriptionSelectorOverride;
    seriesGenreSelectorOverride;
    seriesStatusSelectorOverride;
    seriesAltNameSelectorOverride;
    chapterSelectorOverride;
    chapterNameSelectorOverride;
    chapterDateSelectorOverride;
    pageSelectorOverride;
    // MangaThemesiaAlt random-url support.
    randomUrl;
    searchTitleSelectorOverride;
    static SLUG_PREFIX_REGEX = /^(\d+-)/;
    static URL_MAP_TTL_MS = 60 * 60 * 1000; // 1 hour
    ...
    constructor(config) {
        ...
        this.mangaUrlDirectory = (config.mangaUrlDirectory ?? "/manga").replace(/^\/+|\/+$/g, "");
        ...
        this.requestManager = new MangaThemesiaInterceptor("main", () => this.baseUrl);
    }
    ...
    async getMangaListItems(metadata, order, itemType) {
        ...
        $(this.searchMangaSelector).each((_, element) => {
            ...
            const title = ((this.searchTitleSelectorOverride
    ...
    // Search
    // ----------------------------------------------------------------
    get searchMangaSelector() {
        return (this.discoverItemSelectorOverride ??
    ...
    hasNextBrowsePage($) {
        return ($("div.pagination .next, div.hpage .r, a:has(img[alt=Next])").length > 0);
    }
    ...
    buildBrowseUrl(opts) {
        ...
        if (this.useAdvancedSearchParams) {
            ...
            if (opts.status)
                builder.addQuery("status", opts.status);
            if (opts.type)
                builder.addQuery("type", opts.type.toLowerCase());
        ...
        if (opts.order)
            builder.addQuery("order", opts.order);
        if (opts.author)
            builder.addQuery("author", encodeURIComponent(opts.author));
        if (opts.year)
            builder.addQuery("yearx", encodeURIComponent(opts.year));
        if (opts.status)
            builder.addQuery("status", opts.status);
        if (opts.type)
            builder.addQuery("type", opts.type);
        return builder.build();
    }
    // Maps standard discover/sort tokens to Comic-Asura's `sort` values.
    mapAdvancedOrder(order) {
        switch (order) {
            case "popular":
                return "rating";
            case "update":
    ...
    async getSearchResults(query, metadata, sortingOption) {
        ...
        const filterOrderBy = searchMeta?.orderBy && searchMeta.orderBy.length > 0
        ...
        const url = this.buildBrowseUrl({
            ...
            status: searchMeta?.status && searchMeta.status.length > 0
                ? searchMeta.status[0]
                : undefined,
            type: searchMeta?.type && searchMeta.type.length > 0
                ? searchMeta.type[0]
                : undefined,
        });
        const $ = await this.fetchCheerio({ url, method: "GET" });
    ...
    async getMangaDetails(mangaId) {
        ...
        const url = new URLBuilder(this.baseUrl)
            ...
            .build();
        const $ = await this.fetchCheerio({ url, method: "GET" });
        const details = $(this.seriesDetailsSelectorOverride ??
            "div.bigcontent, div.animefull, div.main-info, div.postbody").first();
        const scope = details.length > 0 ? details : $("html");
        const title = scope
            .find(this.seriesTitleSelectorOverride ??
            "h1.entry-title, .ts-breadcrumb li:last-child span")
            .first()
            .text()
            .trim();
        const image = this.imageFromElement(scope
            .find(this.seriesThumbnailSelectorOverride ??
            ".infomanga > div[itemprop=image] img, .thumb img")
            .first());
        let description = "";
        scope
            .find(this.seriesDescriptionSelectorOverride ??
        ...
        const altName = scope
            .find(this.seriesAltNameSelectorOverride ??
        ...
        const authors = [];
        scope
            .find(".infotable tr:contains(Author) td:last-child, .tsinfo .imptdt:contains(Author) i, .fmed b:contains(Author)+span")
            .each((_, el) => {
            const a = $(el).text().trim();
            if (a && a !== "-" && a.toLowerCase() !== "n/a")
                authors.push(a);
        });
        const artists = [];
        scope
            .find(".infotable tr:contains(Artist) td:last-child, .tsinfo .imptdt:contains(Artist) i, .fmed b:contains(Artist)+span")
            .each((_, el) => {
            const a = $(el).text().trim();
            if (a && a !== "-" && a.toLowerCase() !== "n/a")
                artists.push(a);
        ...
        const genres = [];
        scope
            .find(this.seriesGenreSelectorOverride ??
        ...
        const statusText = scope
            .find(this.seriesStatusSelectorOverride ??
    ...
    async getChapters(sourceManga) {
        ...
        $(this.chapterSelectorOverride ??
            "div.bxcl li, div.cl li, #chapterlist li, ul li:has(div.chbox):has(div.eph-num)").each((_, element) => {
            const el = $(element);
    ...
    async resolveMangaSlug(mangaId) {
        if (!this.randomUrl)
            return mangaId;
        await this.ensureUrlMap();
        const permaSlug = mangaId.replace(MangaThemesiaExtension.SLUG_PREFIX_REGEX, "");
    ...
    async ensureUrlMap() {
        ...
        try {
            ...
            $("div#content div.soralist ul li a.series").each((_, element) => {
                ...
                if (!slug)
                    return;
                const permaSlug = slug.replace(MangaThemesiaExtension.SLUG_PREFIX_REGEX, "");
